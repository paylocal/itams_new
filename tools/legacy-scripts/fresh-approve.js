...
export async function POST(
  ...
  try {
    ...
    if (decision === "REJECTED") {
      ...
      // Update step hien tai - tao moi neu chua co
      ...
      try {
        if (request.requester && request.requester.email) {
          await sendEmail({
            ...
            html: "<p>Yeu cau <b>" + request.requestNumber + "</b> bi tu choi.</p><p>Ly do: " + comment + "</p>",
          });
        }
        if (
          request.requester &&
          ...
          await sendEmail({
            ...
            subject: "[ITAMS] YC " + request.requestNumber + " bi tu choi",
            html: "<p>Yeu cau <b>" + request.requestNumber + "</b> bi tu choi boi " + (session.user.name || "") + ".</p><p>Ly do: " + comment + "</p>",
          });
        }
      } catch (e) {
        ...
        if (newStatus === "ORDERED") {
          if (request.requester && request.requester.email) {
            await sendEmail({
              ...
              html: "<p>Yeu cau <b>" + request.requestNumber + "</b> da hoan thanh.</p>",
            });
          }
          ...
          for (const p of purchasingUsers) {
            if (p.email) {
              await sendEmail({
                ...
                html: "<p>Co YC moi can mua hang: <b>" + request.requestNumber + "</b></p>",
              });
            }
          }
