  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold">
            {locale === "vi" ? "Kho Tai San" : "Asset Inventory"}
          </h1>
          <p className="text-gray-500 mt-1">{filteredAssets.length} / {assets.length} {locale === "vi" ? "tai san" : "assets"}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        <StatBox
          label={locale === "vi" ? "Tong cong" : "Total"}
          value={stats.total}
          color="blue"
        />
        <StatBox
        ...
        />
        <StatBox
          label={locale === "vi" ? "Da cap phat" : "Assigned"}
          value={stats.assigned}
          color="purple"
        />
        <StatBox
          label={locale === "vi" ? "Bao tri" : "Maintenance"}
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        <div className="grid grid-cols-3 gap-3">
          <div className="relative col-span-2">
            ...
            <input
              type="text"
              placeholder={locale === "vi" ? "Tim theo ma, ten, serial, nguoi su dung..." : "Search by tag, name, serial, holder..."}
              value={searchTerm}
          ...
          >
            <option value="ALL">{locale === "vi" ? "Tat ca trang thai" : "All status"}</option>
            <option value="NEW">{locale === "vi" ? "Moi" : "New"}</option>
            <option value="IN_STOCK">{locale === "vi" ? "Trong kho" : "In Stock"}</option>
            <option value="ASSIGNED">{locale === "vi" ? "Da cap phat" : "Assigned"}</option>
            <option value="IN_MAINTENANCE">{locale === "vi" ? "Bao tri" : "Maintenance"}</option>
          </select>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {filteredAssets.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            ...
            <p>{locale === "vi" ? "Khong co tai san" : "No assets found"}</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="text-left p-3">{locale === "vi" ? "Ma TS" : "Asset Tag"}</th>
                <th className="text-left p-3">{locale === "vi" ? "Ten" : "Name"}</th>
                <th className="text-left p-3">{locale === "vi" ? "Danh muc" : "Category"}</th>
                <th className="text-left p-3">{locale === "vi" ? "Trang thai" : "Status"}</th>
                <th className="text-left p-3">{locale === "vi" ? "Nguoi su dung" : "Holder"}</th>
                <th className="text-left p-3">{locale === "vi" ? "Vi tri" : "Location"}</th>
...
const dir = path.join(__dirname, "src", "components", "assets");
const file = path.join(dir, "assets-list.tsx");
