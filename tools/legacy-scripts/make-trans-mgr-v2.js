...
export function TranslationManager() {
  ...
  async function deleteKey(key: string) {
    ...
    try {
      const res = await fetch(
        "/api/admin/translations?languageCode=" + selectedLang + "&key=" + encodeURIComponent(key),
        { method: "DELETE" }
      );
      if (res.ok) {
  ...
  // Lay tat ca keys (gom tu translations + keys mac dinh neu chua co)
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Sua noi dung dich cho tung ngon ngu (luu tu dong)
          </p>
        </div>
      </div>
