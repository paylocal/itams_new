...
export async function POST(
  ...
  try {
    ...
    // Helper: update hoac tao step moi
    async function upsertStep(stepNum, data) {
    ...
    if (decision === "REJECTED") {
      ...
      try {
        ...
        if (request.requester && request.requester.manager && request.requester.manager.email) {
        ...
        for (const pUser of purchasingUsers) {
          if (pUser.email) {
            const t2 = emailTemplates.requestApproved({
              ...
              role: "Purchasing",
              url: (process.env.NEXTAUTH_URL || "http://localhost:3000") + "/purchase-orders/select-items",
            });
            await sendEmail({
              to: pUser.email,
    ...
    return NextResponse.json(
      { error: "Loi server: " + (error && error.message ? error.message : "Unknown") },
      { status: 500 }
    );
  }
}
...
console.log("Created/Updated approve route");