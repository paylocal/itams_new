...
interface Props {
  stats: { total: number; pending: number; completed: number; myAssets: number };
  role: string;
}
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">
          {t("dashboard.welcome")}, {role === "ADMIN" ? "Admin System" : roleLabels[role]}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <StatCard
          icon={Package}
          label={role === "EMPLOYEE" ? t("dashboard.myAssets") : t("nav.assets")}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {role === "EMPLOYEE" && (
            ...
            >
              ...
              <p className="font-medium text-blue-700">{t("nav.createRequest")}</p>
            </Link>
          )}
          {role === "MANAGER" && (
            <Link
            ...
            >
              ...
              <p className="font-medium text-yellow-700">{t("nav.approvals")}</p>
            </Link>
          )}
        </div>
      </div>
