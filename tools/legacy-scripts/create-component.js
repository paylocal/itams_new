...
export function CreatePOFormMulti({ availableRequests, preselected }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(\`Tao PO thanh cong voi \${selectedIds.length} yeu cau!\`);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-4">
      ...
      <h1 className="text-2xl font-bold">Tao don mua hang (PO) - Nhieu yeu cau</h1>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-white p-6 rounded-lg shadow">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-bold">Chon yeu cau ({selectedIds.length} da chon)</h2>
            <input
              type="text"
          ...
          {filteredRequests.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              Khong co yeu cau nao dang cho
            </div>
          ) : (
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {filteredRequests.map((req) => {
                const isSelected = selectedIds.includes(req.id);
                const isPreselected = preselected.find((p) => p.id === req.id);
                return (
                  <div
                    key={req.id}
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500">
                          {req.items.length} mat hang | Tong: {formatCurrency(req.totalAmount || 0)}
                        </p>
                      </div>
                      {isSelected && (
        ...
        <form
          ...
          className="bg-white p-6 rounded-lg shadow space-y-3 h-fit sticky top-4"
        >
            ...
            <div className="bg-blue-50 border border-blue-200 rounded p-3">
              ...
              <div className="flex flex-wrap gap-1 mt-2">
                {selectedRequests.map((r) => (
                  <span
                    key={r.id}
                    className="bg-white border rounded px-2 py-0.5 text-xs flex items-center gap-1"
                  >
                    {r.requestNumber}
                    <X
          ...
          <div>
            ...
            />
            {poFile && (
              <p className="text-xs text-green-700 mt-1">{poFile.name}</p>
            )}
          </div>
          ...
          {error && (
            <div className="bg-red-50 text-red-700 p-2 rounded text-xs flex items-center gap-1">
          ...
          >
            ...
            {loading ? "Dang tao..." : \`Tao PO (\${selectedIds.length} YC)\`}
          </button>
        </form>
      </div>
    </div>
