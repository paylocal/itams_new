...
import { ShoppingCart, Package, FileText, ChevronDown, ChevronUp } from "lucide-react";
          ...
          {purchaseOrders.map((po) => {
            ...
            return (
              <div key={po.id} className="bg-white p-4 rounded-lg shadow">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500">
                      {po.items.length} mat hang tu {po.requests.length} yeu cau
                    </p>
                  </div>
                ...
                {/* DANH SACH ITEMS TRONG PO */}
                <div className="mt-3 pt-3 border-t">
                  <p className="text-xs font-medium text-gray-600 mb-2">
                    Chi tiet mat hang ({po.items.length}):
                  </p>
                  <div className="space-y-1">
                    {po.items.map((item) => {
                      const req = po.requests.find(
                        (r) => r.requestId === item.requestItem?.requestId
                      );
                      return (
                        <div
                          key={item.id}
                        ...
                        >
                          <span className="flex-1">
                            {req && (
                              <Link
                                ...
                                className="text-blue-600 hover:underline mr-1 font-mono\`}
                              >
                          ...
                          <span className="font-medium text-blue-700 w-32 text-right">
                ...
                {/* DANH SACH YC THUOC PO */}
                <div className="mt-3 pt-3 border-t">
                  <p className="text-xs font-medium text-gray-600 mb-2">
                    Yeu cau nguon ({po.requests.length}):
                  </p>
...
const file = path.join(__dirname, "src", "app", "(dashboard)", "purchase-orders", "page.tsx");
