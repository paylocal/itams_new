...
const defaultConfig: EmailConfig = {
  ...
  from: process.env.SMTP_FROM || "Quản lý yêu cầu và tài sản <noreply@company.com>",
...
export const emailTemplates = {
  requestCreated: (data: {
    ...
    return {
      subject: isVi
        ? `[Quản lý yêu cầu và tài sản] Yeu cau moi can duyet: ${data.requestNumber}`
        : `[Quản lý yêu cầu và tài sản] New request needs approval: ${data.requestNumber}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: linear-gradient(135deg, #3b82f6, #1e40af); color: white; padding: 20px; border-radius: 8px 8px 0 0;">
            <h2 style="margin: 0;">Quản lý yêu cầu và tài sản - ${isVi ? "Yeu cau moi" : "New Request"}</h2>
          </div>
  ...
  requestApproved: (data: {
    ...
    return {
      subject: isVi
        ? `[Quản lý yêu cầu và tài sản] ${data.requestNumber} can buoc ${data.role} duyet`
        : `[Quản lý yêu cầu và tài sản] ${data.requestNumber} needs ${data.role} approval`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: linear-gradient(135deg, #3b82f6, #1e40af); color: white; padding: 20px; border-radius: 8px 8px 0 0;">
            <h2 style="margin: 0;">Quản lý yêu cầu và tài sản</h2>
          </div>
          <div style="background: #f9fafb; padding: 20px; border: 1px solid #e5e7eb;">
  ...
  requestRejected: (data: {
    ...
    return {
      subject: isVi
        ? `[Quản lý yêu cầu và tài sản] Yeu cau ${data.requestNumber} bi tu choi`
        : `[Quản lý yêu cầu và tài sản] Request ${data.requestNumber} rejected`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
  ...
  requestCompleted: (data: {
    ...
    return {
      subject: isVi
        ? `[Quản lý yêu cầu và tài sản] Yeu cau ${data.requestNumber} da hoan thanh`
        : `[Quản lý yêu cầu và tài sản] Request ${data.requestNumber} completed`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
  ...
  poCreated: (data: {
    ...
    return {
      subject: isVi
        ? `[Quản lý yêu cầu và tài sản] PO moi ${data.poNumber} cho IT`
        : `[Quản lý yêu cầu và tài sản] New PO ${data.poNumber} for IT`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: linear-gradient(135deg, #f59e0b, #d97706); color: white; padding: 20px; border-radius: 8px 8px 0 0;">
            <h2 style="margin: 0;">Quản lý yêu cầu và tài sản - ${isVi ? "Don mua hang moi" : "New Purchase Order"}</h2>
          </div>
          <div style="background: #f9fafb; padding: 20px; border: 1px solid #e5e7eb;">
  ...
  passwordReset: (data: {
    ...
    return {
      subject: isVi
        ? `[Quản lý yêu cầu và tài sản] Mat khau tai khoan cua ban da duoc dat lai`
        : `[Quản lý yêu cầu và tài sản] Your account password has been reset`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: linear-gradient(135deg, #3b82f6, #1e40af); color: white; padding: 20px; border-radius: 8px 8px 0 0;">
            <h2 style="margin: 0;">Quản lý yêu cầu và tài sản - ${isVi ? "Mat khau moi" : "New Password"}</h2>
          </div>
          <div style="background: #f9fafb; padding: 20px; border: 1px solid #e5e7eb;">
