...
import { ArrowLeft, Check, ShoppingCart, CheckCircle2 } from "lucide-react";
...
export function SelectItemsForPO({ availableRequests }: Props) {
  ...
  const handleCreatePO = () => {
    if (selectedItems.size === 0) {
      alert(t("po.selectAtLeastOne", "Please select at least one item"));
      return;
    }
    ...
    return (
      <div className="max-w-6xl mx-auto space-y-4">
        ...
        <div className="bg-white p-12 rounded-lg shadow text-center text-gray-500">
          ...
          <p className="font-medium">{t("po.allCompleted", "All requests completed!")}</p>
          <p className="text-xs mt-2">{t("po.noItemsToPurchase", "No items left to purchase")}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-4">
      ...
      <div>
        <h1 className="text-2xl font-bold">{t("po.selectItemsTitle", "Select items for PO")}</h1>
        <p className="text-gray-500 mt-1">
          {visibleRequests.length} {t("po.requestsNeedItems", "requests have items to purchase")}
        </p>
      </div>
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded">
                      {t("po.itemsRemaining", "{{n}} items left", { n: req.items.length })}
                    </span>
                  </div>
                  <p className="text-sm">{req.title}</p>
                  <p className="text-xs text-gray-500">
                    {t("request.requester", "Requester")}: {req.requester.name}
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-start">
                            ...
                            <div className="text-right">
                              <p className="text-sm">
                                {t("common.quantity", "Qty")}: <strong>{item.quantity}</strong>
                              </p>
                              <p className="text-xs text-gray-500">
                                {formatCurrency(item.unitPrice || 0)}/{t("common.each", "each")}
                              </p>
          ...
          <div className="flex justify-between items-center">
            <div>
              <p className="text-sm text-gray-600">
                {t("po.selectedCount", "Selected")}:{" "}
                <strong className="text-blue-700">
                  {getTotalSelectedCount()} {t("common.items", "items")}
                </strong>
              </p>
              <p className="text-2xl font-bold text-blue-700">
                {t("common.total", "Total")}: {formatCurrency(getTotalSelected())}
              </p>
              {willComplete.length > 0 && (
                <p className="text-xs text-green-600 mt-1">
                  {t("po.willComplete", "Will complete")}: {willComplete.join(", ")}
                </p>
              )}
            </div>
            <button
            ...
            >
              ...
              {t("po.create", "Create PO")} ({getTotalSelectedCount()})
            </button>
          </div>
        </div>
      </div>
