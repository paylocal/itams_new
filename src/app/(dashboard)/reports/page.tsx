...
export default async function ReportsPage() {
  ...
  recentPOs.forEach((p) => {
    ...
    const amount = typeof p.totalAmount === "number" ? p.totalAmount : p.totalAmount?.toNumber?.() ?? 0;
    monthlyCost[month] = (monthlyCost[month] || 0) + amount;
  });
  ...
  return (
    <ReportsDashboard
      stats={{
        ...
        totalPOs,
        totalSpent:
          typeof totalSpent._sum.totalAmount === "number"
            ? totalSpent._sum.totalAmount
            : totalSpent._sum.totalAmount?.toNumber?.() ?? 0,
      }}
