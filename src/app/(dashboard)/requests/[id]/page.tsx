...
export default async function RequestDetailPage({
  ...
  const request = await prisma.assetRequest.findUnique({
    where: { id: params.id },
    include: {
      requester: { include: { manager: true } },
      approvalSteps: {
        include: { approver: true },
        orderBy: { stepNumber: "asc" },
      },
      items: {
        include: { category: true, deviceModel: true },
      },
      purchaseOrders: {
        include: {
          purchaseOrder: { select: { poNumber: true, id: true, status: true } },
        },
      },
    },
  });
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        ...
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div
            ...
            style={{
              width:
                totalItems > 0 ? (itemsWithPO / totalItems) * 100 + "%" : "0%",
            }}
          ></div>
        </div>
      </div>
